...
/* no `opacity: 0` in the base rule: if a renderer ignores CSS animations the
   cells must still be visible. `both` supplies the 0% state during the delay. */
...
""".strip()
    ...
    parts.append(f'<text xml:space="preserve" x="{PAD}" y="{ly}" font-size="13" fill="{GREEN}">'
    ...
    ly += 24
    parts.append(f'<text xml:space="preserve" x="{PAD}" y="{ly}" font-size="13" fill="{MUTED}">current streak '
